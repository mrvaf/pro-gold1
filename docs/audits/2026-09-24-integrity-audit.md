...
- داده‌ها در حافظه (in-memory) نگه داشته می‌شوند؛ در کل تاریخچه هیچ‌وقت درایور PostgreSQL نصب نشده. **رفع شد (Stage 8.3 — ADR-0047/0048/0049):** درایور `pg` نصب و سیم‌کشی شد (`createPersistence` با کلید `DATABASE_ENABLED=true`)؛ migrationهای 0001–0012 با runner ترتیبی و ledger روی خوشه‌ی PostgreSQL واقعی اعمال شد؛ RLS روی هر ۱۲ جدول tenant-scoped (سیاست شرطی + context تراکنشی `app_tenant_id`).
