...
## 3. Technology Stack (Stage 4 Implemented)
...
* **Testing:** Vitest 3.x (31 test suites, 140 passing tests)
* **Validation:** Zod
...
│   │   └── src/ports/         # Ports (User, TenantMembership, Session, Hasher, MarketDataProvider, Repos)
│   ├── database/              # DB infrastructure & Drizzle ORM
│   │   ├── src/schema/        # Drizzle tables (tenants, stores, users, memberships, sessions, market data)
│   │   ├── src/migrations/    # Numbered DDL migrations (0001_core, 0002_iam, 0003_market_data)
...
├── tests/                     # 31 Vitest test suites (140 tests passed)
...
* **Status:** **COMPLETE**
* **Next Stage:** **Stage 4.1 (Financial Precision & Currency Semantics) or Stage 5** — *Awaiting user prompt.*
...
```bash
# Run full Vitest test suite (140 tests passed across 31 test suites)
npm test

# Run strict TypeScript typecheck across all workspaces and tests
npm run typecheck

# Run production build for all packages and Next.js web application (10 routes)
npm run build
```
