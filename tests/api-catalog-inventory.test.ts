...
describe('Catalog & Inventory Web API Endpoints', () => {
  ...
  describe('Catalog Products API', () => {
    it('creates and retrieves a product via API', async () => {
      ...
      // 2. GET product by ID
      const getReq = new NextRequest(`http://localhost:3000/api/v1/catalog/products/${productId}?tenantId=${tenantId}`);
      ...
      // 3. GET product by ID with other tenant -> 403 Forbidden
      const idorReq = new NextRequest(`http://localhost:3000/api/v1/catalog/products/${productId}?tenantId=${otherTenant}`);
    ...
    it('lists products scoped to tenant', async () => {
      const listReq = new NextRequest(`http://localhost:3000/api/v1/catalog/products?tenantId=${tenantId}`);
  ...
  describe('Inventory Locations, Items & Movements API', () => {
    ...
    it('intakes discrete inventory item, transitions status to RESERVED, and verifies movement log', async () => {
      ...
      const varListReq = new NextRequest(`http://localhost:3000/api/v1/catalog/variants?tenantId=${tenantId}`);
      ...
      const moveReq = new NextRequest(`http://localhost:3000/api/v1/inventory/movements?tenantId=${tenantId}&itemId=${itemId}`);
